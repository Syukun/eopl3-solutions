#lang eopl
;; Modify the unifier so that it calls apply-subst-to-type only on type variables, rather than on its arguments.
...
;; 题目的要求是修改unifier函数，使得该函数只对type variable调用apply-subst-to-type 
;; unifier : Type * Type * Subst * Exp -> Subst
(define unifier ;; 这个函数的功能是什么呢? 我想了一下,这个函数的主要功能是将equations里面的式子添加入substitution里面的
  (lambda (ty1 ty2 subst exp)
    (let ([ty1 (if (tvar-type? ty1) (apply-subst-to-type ty1 subst) ty1)] ;; 好吧，修改在这里，感觉修改得不咋的。
