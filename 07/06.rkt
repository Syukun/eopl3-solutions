...
(define type-of
  (lambda (exp tenv)
    (cases expression exp
      ...
      ;; 所谓的赋值语句，我是这样理解的，那就是x如果是int类型，那么set x = exp
      ;; exp也必须是int类型，然后赋值语句返回值是int类型
      (assign-exp (id exp1)
                  (let ([exp1-type (type-of exp1 tenv)])
                    (check-equal-type! exp1-type (apply-tenv tenv id) exp1)
                    (int-type))) 
      )))
...
;; test code
(type-of-program (scan&parse "letrec bool g (x:int) = zero?(x) in (g 2)"))
(type-of-program (scan&parse "letrec int f (x:int) = x ; bool g (x:int) = zero?(x) in 1"))
