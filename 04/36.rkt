#lang eopl
;; Add a call-by-reference version of let, called letref, to the language.
    ...
    (cases expression exp
      ...
      (newarray-exp (exp1 exp2)
                   (letrec [(size (expval->num (value-of exp1 env)))
                            ...
                            (create-array
                             (lambda (init-num count)
                               (if (= 1 count) (newref init-num) ;; 建立一个新的
                                   (begin
                                     (newref init-num)
                                     (create-array init-num (- count 1))))))]
                     (let [(start (newref val))]
                       (begin
                         (print "newarry-exp start -> " start)
                         (create-array val (- size 1))
                         ;(print "newarray-exp val -> " val)
                         (array-val start size)))))
      ...
      (arraylength-exp (exp1)
                       (let [(length (expval->array-length (value-of exp1 env)))]
                         (num-val length)))
      ;; ref
      (letref-exp (id exp1 body)
                  (cases expression exp1
                    (var-exp (var)
                             (value-of body (extend-env id (apply-env env var) env)))
                    (else
                     (let [(val (value-of exp1 env))]
                       (value-of body (extend-env id (newref val) env))))))
      (ref-exp (id)
               (ref-val (apply-env env id)))
...
;; test code
(run "let a = newarray(10, 3) b = newarray(10, 4)
         swap = proc (x, y)
           let temp = deref (x)
             in begin
                setref(x, deref(y));
       ...
       in begin (swap arrayref(a, 1) arrayref(b, 1)); deref(arrayref(a, 1)) end")
;; ==> (num-val 4)
