#lang eopl
;; Add new feature to our interpreter.
...
(define-datatype environment environment?
  ...
  (extend-env
   (bvar symbol?)
   (bval reference?) ;; bval居然要求是一个应用，好吧！
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (call-exp (rator rand) ;; 如果要实现setdynamic的话，我有一个想法，那就是设置一个变量，表示是否在计算时在闭包内注入新的绑定
      ...
      (setdynamic-exp (var exp1 body)
                      (begin
                        (print "setdynamic-exp var -> " var)
                        (print "setdynamic-exp exp1 -> " exp1)
                        (print "setdynamic-exp body -> " body)
                        (let [(new-ref (newref (value-of exp1 env)))]
                          (begin
                            (set! dynamic-flag (list #t var new-ref)) ;; 设置dynamic-flag标志
                            (let [(res (value-of body (extend-env var new-ref env)))]
                              (begin
                                (set! dynamic-flag '(#f)) ;; 结果运算完成之后要取消这个标志
                                res))))
                        ))
      (else (eopl:error "bad match"))
                  
      )))
(define dynamic-flag '(#f))
  ...
  (lambda (proc1 val-list)
    (cases proc proc1
	   (procedure (var-list body saved-env) ;; var表示变量名
                      (value-of body
                                (let [(new-env (extend-env-list var-list val-list saved-env))]
                                 (if (car dynamic-flag)
                                     (begin
                                       (print "(cadr dynamic-flag) -> " (cadr dynamic-flag))
                                       (print "(caddr dynamic-flag) -> " (caddr dynamic-flag))
                                       (extend-env (cadr dynamic-flag) (caddr dynamic-flag) new-env))
                                     new-env))
                                )))))
...
;; test code
(run "let p = proc (y) -(y, x) in setdynamic x = 17 during (p 22)")
;; ==> (num-val 5)
