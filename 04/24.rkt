...
(define the-grammar
  '((program (statement) a-program)
    ...
    (statement ("var" (separated-list identifier ",") ";" statement) declare-stat)
    (statement ("do" statement "while" expression) do-while-stat)
    ))
  ...
  (lambda (stat) ;; 
    (cases statement stat
      ...
      (do-while-stat (stat1 exp1)
                     (letrec [(interpreter-do-while
                               (lambda ()
                                 (begin
                                   (value-of-stat stat1) ;; 先执行一次
                                   (if (expval->bool (value-of exp1 global-env))
                                       (interpreter-do-while)
                                       (num-val 1)))))]
                       (interpreter-do-while)))
      
      (declare-stat (var-list stat1)
                    (begin
                      (let [(backup global-env)] ;; 先备份起来
                        (begin
                          (extend-env-with-var-list var-list)
...
;; test code
(run "var x; {x = 3;do {print x; x = -(x, 1)} while not(zero?(x))}")
