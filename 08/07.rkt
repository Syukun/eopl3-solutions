...
;; 好吧，实在要实现的话，是可以的，不过工作量实在大了一些，我这里只实现了计算正确，但是没有实现类型检查
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; parse ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define the-grammar
  '((program ((arbno module-definition) expression) a-program)
    ...
    (module-body ((arbno module-definition) "[" (arbno definition) "]") embeded-module-body)
    (type ("[" identifier ":" type "]" ) module-type)
    ;(opt-type (type) common-type)
    
    (definition (identifier "=" expression) val-defn)
    (expression ("from" identifier (arbno "take" identifier)) qualified-var-exp) ;; 好吧，其实可以取多次的
...
;; 这个函数要大大的改造啊!估计也是递归调用,好吧,lisp强大异常啊!
...
(define value-of-module-body
  (lambda (m-body env)
    (cases module-body m-body
      (embeded-module-body (modules defns) ;; modules的东西是嵌套的
                           (if (null? modules)
                               (simple-module (defns-to-env defns env))
                               ;; 其实也非常简单啦,defns-to-env函数其实得到的是一个env
                               (let ([new-env (add-module-defns-to-env modules env)]) ;; 好吧,其实只是一个很简单的递归而已
                                 (simple-module (defns-to-env-with-module modules defns new-env))
                                 )))
...
;; defns-to-env-with-module
(define defns-to-env-with-module
  (lambda (modules defns env)
    (if (null? defns)
        (empty-env)
        (cases definition (car defns)
          (val-defn (var exp)
                    ;; 在计算val之前,我们有必要弄一个判断exp是不是一个module的名字,如果是的话,那么要构造一个module-val
                    (let ([maybe-a-module (module-name?&get-module exp modules)])
                      (eopl:printf "defns-to-env-with-module :\n maybe-a-modue -> ~a\n" maybe-a-module)
                      (if maybe-a-module
                          ;; 我们需要构造一个module-val
                          (let ([saved-env (add-module-defns-to-env (list maybe-a-module) (empty-env))])
                            (eopl:printf "defns-to-env-with-module :\n saved-env -> ~a\n" saved-env)
                            (extend-env var (module-val saved-env) (defns-to-env
                                                                     (cdr defns) env)))
                          ;; 如果exp表示的不是一个module的名字的话
                          (let ([val (value-of exp env)])
                            (let ([new-env (extend-env var val env)])
                              (extend-env var val
                                          (defns-to-env ;; 递归调用
                                            (cdr defns) new-env)))))))))))
                        ;; 如果是一个module的名字的话，那么，我们有必要找到这个变量的类型啊。
              

;; module-name
;; 判断一个东西是不是module名
(define module-name?&get-module
  (lambda (var modules)
    (let ([var-exp?
           (lambda (exp)
             (cases expression exp
               ...
               (else #f)))])
      (letrec ([get-module
                (lambda (md-name modules)
                  (if (null? modules) #f
                      (cases module-definition (car modules)
                        (a-module-definition (m-name iface m-body)
                                             (eopl:printf "module-name?&get-module :\n m-name -> ~a\n md-name -> ~a\n" m-name md-name)
                                             (if (eqv? md-name m-name) (car modules) ;; 返回这个module
                                                 (get-module md-name (cdr modules)))))))])
      (let ([maybe-module-name (var-exp? var)])
        (if maybe-module-name (get-module maybe-module-name modules) #f))))))
...
(define interface-of
  (lambda (m-body tenv)
    (cases module-body m-body
      ...
      (embeded-module-body (modules defns)
                           (if (null? modules)
                               (simple-iface (defns-to-decls defns tenv))
                               (let ([new-tenv (add-module-defns-to-tenv modules tenv)])
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; simple-exp ;;;;;;;;;;;;;;;;;;;;;;;;;;
;; 新的玩意儿
 (define lookup-var-in-env
        (lambda (var env)
          (cases environment env
            (empty-env ()
		      (eopl:error 'lookup-var-in-env
				  "No module binding for ~s" var))
            (extend-env (bvar bval saved-env)
                        (lookup-var-in-env var saved-env))
    ...
    (cases expression exp
      ...
      (qualified-var-exp (m-name var-names)
                              ...
                              (let ([val (lookup-qualified-var-in-env m-name (car var-names) env)])
                                (letrec ([find-recursively
                                          (lambda (var rest-vars env);; 查找一个module的名字，并在该module中找到对应的值
                                            (let ([maybe-answer (lookup-var-in-env var env)])
                                              (if (null? rest-vars) maybe-answer
                                                (cases expval maybe-answer
                                                  (module-val (env) (find-recursively (car rest-vars) (cdr rest-vars) env))
                                                  (else (eopl:error "There are something wrong with the answer!\n"))))))])            
                                ;; 首先找到一个对应的东西
                                  (if (null? (cdr var-names)) val
                                      (cases expval val
                                        (module-val (saved-env)
                                                    (find-recursively (cadr var-names) (cddr var-names) saved-env))
                                        (else (eopl:error "There are something wrong with the answer!\n"))
                                              )))))
...
(define type-to-external-form
  (lambda (ty)
    (cases type ty
      ...
      (module-type (id ty)
                   (list 'module id ': (type-to-external-form ty)))
	   )))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; data-structures ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (lambda (m-name env)
    (cases environment env
	   ...
	   (extend-env-recursively  (id bvar body saved-env)
				    (lookup-module-name-in-env m-name saved-env))
	   (extend-env-with-module (m-name1 m-val saved-env)
	    (if (eqv? m-name1 m-name) ;; 如果模块的名字相等
		m-val ;; 返回m-val话说这玩意到底是什么
		(lookup-module-name-in-env m-name saved-env))))))
...
(define lookup-qualified-var-in-env ;; qualified var究竟是什么鬼? 有意思啊。
  (lambda (m-name var-name env) 
    (let ((m-val (lookup-module-name-in-env m-name env))) ;; 先查找模块的名字，在模块里查找出对应的东西
      (cases typed-module m-val
