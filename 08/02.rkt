          ...
          (a-module-definition (m-name iface m-body) ;; 好吧,这就是一个module的名字,然后是声明的变量,最后是实现
                               (add-module-defns-to-env ;; 先加载一个module,然后加载余下的module,好吧,这就是所谓的递归
                                ...
                                (extend-env-with-module
                                 m-name ;; module的名字
                                 ;; 这个东西确实存在这缺陷，那就是在env中加入了不相干的东西
                                 (value-of-module-body-m iface m-body env) ;; module里面包含的东西
                                 env)))))))
(define value-of-module-body-m
  (lambda (iface m-body env)
    (cases module-body m-body
      (defns-module-body (defns)
        (cases interface iface
          (simple-iface (decls);; decls是一些定义
                        (simple-module
                         (begin
                           (eopl:printf "value-of-module-body-m :\n env -> ~a " (add-decl-to-env decls defns env))
                           (add-decl-to-env decls defns env)))))))))

 ;; 首先，我们已经做了类型检查，可以保证，decls里面的东西，defns都实现了
(define add-decl-to-env
  (lambda (decls defns env)
    (if (null? decls) (empty-env)
        (cases definition (car defns)
          (val-defn (def-name exp) ;; 这个东西是module body里面的名字和exp
                    (cases declaration (car decls)
                      (val-decl (dec-name ty)
                                (if (equal? def-name dec-name) ;; 变量名相同
                                    (let ([val (value-of exp env)])
                                      (let ([new-env (extend-env def-name val env)])
                                        (extend-env def-name val
                                                    (add-decl-to-env (cdr decls) (cdr defns) new-env)))) 
                                      (add-decl-to-env decls (cdr defns) env)))))))))
  ...
  (lambda (m-body env)
    (cases module-body m-body
      (defns-module-body (defns) ;; 这里的defns应该是一个list
        (simple-module ;; simple-module 是一个类型
...
;; test code
(run "module m interface [a:int] body [b = 2 a=1] 1")
