#lang eopl
;; 这道题目也是很简单的一道题目，具体的要求也是实现函数重载，不过有所不同的是，我们要预先经过一个预处理的过程
;; 好吧，其实很简单,我这里实际只修改了很小很小的一个部分，只是将函数的名字改了，你可以仔仔细细看我做了一些什么东西。
      ...
      (let-exp (vars exps body)
	   ...
	   (new-object-exp (class-name rands)
			   (let ([args (values-of-exps rands env)]
			     ...
			     (apply-method
			      (find-method class-name (string->symbol (string-append (symbol->string 'initialize)
                                                                                     "@"
	   ...
	   (method-call-exp (obj-exp method-name rands)
			    (let ((args (values-of-exps rands env))
			      ...
			      (apply-method
			       (find-method (object->class-name obj) (string->symbol (string-append (symbol->string method-name)
                                                                                                    "@"
                                                                                                    (number->string (length args)))))
			       obj
			       args)))

	   (super-call-exp (method-name rands) ;; 好吧，这里也要做相应的修改
			   (let ([args (values-of-exps rands env)]
				 [obj (apply-env env '%self)])
			     (apply-method
			      (find-method (apply-env env '%super) (string->symbol (string-append (symbol->string method-name)
                                                                                                   "@"
                                                                                                   (number->string (length args)))))
			      obj
			      args)))
     ...
     (lambda (m-decl)
       (cases method-decl m-decl
         (a-method-decl (method-name vars body) 
                        (list (string->symbol (string-append (symbol->string method-name)
                                                             "@"
                                                             (number->string (length vars))))  ;; 对函数名变一下名即可
                              (a-method vars body super-name field-names)))))
     m-decls)))
