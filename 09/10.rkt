  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (let-exp (vars exps body)
	   ...
	   (super-call-exp (method-name rands) ;; 方法名，然后是参数
                           (eopl:printf "env -> ~a\n" env)
			   (let ([args (values-of-exps rands env)]
				 [obj (apply-env env '%self)]) ;; 要查找当前对象self
			     (apply-method
			      (find-method (apply-env env '%super) method-name) ;; 查找父类的方法
			      obj
			      args)))
      (named-send-exp (c-name exp1 m-name rands)
                      (let ([obj (value-of exp1 env)])
                        (let ([args (values-of-exps rands env)])
                          (apply-method
                           (find-method c-name m-name) ;; 寻找 class的方法
                           obj
                           args))))
	   )))
