#lang eopl
;; 题目的意思很简单，那就是移除重复的定义。
;; 前面的例子里，对于一个calss，如果其重定义了父类的一个方法，那么它的方法列表里既有子类的该方法，也有父类的该方法，不过这里的话，父类的该方法，我们一般不在这个类里面
;; 寻找，所以题目要求我们在子类的calss里面移除父类的该方法的定义
                       ...
                       (a-class s-name f-names
                                (merge-method-envs
                                 ...
                                 (method-decls->method-env
                                  m-decls s-name f-names))))))))) ;; s-name 父类的名字 f-names field names
...
;; method-decls -> method-env
(define method-decls->method-env ;; 这是非常重要的方法
  (lambda (m-decls super-name field-names)
    (map
     (lambda (m-decl)
       (cases method-decl m-decl
         (a-method-decl (method-name vars body) ;; 方法名 参数 函数体
                        (list method-name ;; 这里构建了一个list 
                              (a-method vars body super-name field-names)))))
     m-decls)))

;; merge-method-envs : MethodEnv * MethodEnv -> MethodEnv
;; 我们要改变的就是这个方法，那就是去除子类中已有的，而父类中也存在的方法的定义
(define merge-method-envs
  (lambda (super-m-env new-m-env)
    (if (null? super-m-env) new-m-env
        (if (method-exist? (car super-m-env) new-m-env)
            (merge-method-envs (cdr super-m-env) new-m-env)
            (merge-method-envs (cdr super-m-env) (append new-m-env (list (car super-m-env))))))))

(define method->method-name
  (lambda (a-method)
    (car a-method)))

;; method-exits? 该函数主要用来判断一个方法method是否在m-env中已经存在定义了
(define method-exist?
  (lambda (method m-env)
    (if (null? m-env) #f
        (or (eqv? (method->method-name method)
                  (method->method-name (car m-env)))
            (method-exist? method (cdr m-env))))))
...
(define apply-env
  (lambda (env search-sym)
    (cases environment env
	   ...
	   (extend-env (bvars bvals saved-env) ;; bvars 表示一大群的变量，bvals代表一大群对应的值
		       (cond
