#lang eopl
;; 这道题目比较简单，那就是重写merge-method-envs
;; 使得，每个方法在class的i的定义中只出现一次。
...
(define merge-method-envs
  (lambda (super-m-env new-m-env)
    (letrec ([f (lambda (s-env m-env n-s-env)
                 (if (null? s-env) (append n-s-env m-env)
                     (let ([a-decl (car s-env)])
                       (let ([m-name (car a-decl)])
                         (let ([maybe-method (get-method-decl m-name m-env)])
                           (if (car maybe-method) ;; 查看这玩意到底存在不存在
                               (let ([new-s-env (replace-method m-name (cadr maybe-method) n-s-env)]
                                     [new-m-env (remove-method m-name m-env)])
                                 (f (cdr s-env) new-m-env new-s-env))
                               (f (cdr s-env) m-env n-s-env)))))))])
    (f super-m-env new-m-env super-m-env))))
                                 

;; 两大利器已经写成了，还怕啥！

(define get-method-decl
  (lambda (name m-env)
        (if (null? m-env) (list #f)
            (let ([a-decl (car m-env)])
              (let ([m-name (car a-decl)])
                (if (eqv? name m-name) (list #t (cadr a-decl))
                    (get-method-decl name (cdr m-env))))))))

(define replace-method
  (lambda (name meth m-env)
    (if (null? m-env) '()
        (let ([a-decl (car m-env)])
          (let ([m-name (car a-decl)])
            (if (null? m-env) (cons (cons m-name meth)
                                    (replace-method name meth (cdr m-env)))
                (cons a-decl (replace-method name meth (cdr m-env)))))))))
...
(define remove-method
  (lambda (name m-env)
    (if (null? m-env) '()
         (let ([a-decl (car m-env)])
           (let ([m-name (car a-decl)])
             (if (eqv? m-name name) (remove-method name (cdr m-env)) ;; 实现了移除该方法
                 (cons (car m-env) (remove-method name (cdr m-env))))))))) 
