      ...
      (let-exp (vars exps body)
	   ...
	   (method-call-exp (obj-exp method-name rands)
			    (let ([args (values-of-exps rands env)]
			      ...
			      (apply-method
			       (find-method (object->class-name obj) method-name) ;; 找到对应的方法
			       obj
			       args)))
...
(define new-object ;; 创建一个新的object
  (lambda (class-name)
    (an-object
     class-name
     (map
  ...
  (lambda (m self args)
    (cases method m
      (a-method (vars body super-name field-names)
                (value-of body
                          (extend-env-with-static-decls self 
...
(define extend-env-with-static-decls
  (lambda (self env)
    (cases object self
      (an-object (c-name fields)
                 (cases class (lookup-class c-name)
                   (a-class (s-name static-decls f-names m-env)
                            (let ([extend-env-with-s-decls
                                   (lambda (s-decls env)
                                    (let ([s-names (map (lambda (a-decl) (car a-decl)) s-decls)]
                                          [s-refs (map (lambda (a-decl) (cdr a-decl)) s-decls)])
                                      (extend-env s-names s-refs env)))])
                              (extend-env-with-s-decls static-decls env))))))))
...
(define static-field?
  (list-of (lambda (p) (and (pair? p) (reference? (cdr p))))))
...
(define initialize-class-decl!
  (lambda (c-decl)
    (cases class-decl c-decl
      (a-class-decl (c-name s-name s-decls f-names m-decls)
                    ...
                    (let ([f-names
                           (append-field-names
                            ...
                            f-names)])
                      (let ([static-decls (get-static-decls s-decls)])
                        (add-to-calss-env!
                         c-name
                         (a-class s-name static-decls f-names
                                  (merge-method-envs
                                   (class->method-env (lookup-class s-name))
                                   (method-decls->method-env
                                    m-decls s-name f-names))))))))))

(define get-static-decls
  (lambda (f-decls)
    (if (null? f-decls) '()
        (cases static-decl (car f-decls)
          (a-static-decl (id exp) (cons (cons id (newref (value-of exp (empty-env))))
                                         (get-static-decls (cdr f-decls))))))))
...
(define class->super-name
  (lambda (c-struct)
    (cases class c-struct
	   (a-class (super-name static-names field-names method-env)
		    super-name))))
...
in list (send o1 get-serial-number(),
         ...
         send o3 get-serial-number())
")
;; ==> (list-val (num-val 1) (num-val 2) (num-val 3))