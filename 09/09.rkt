    ...
    (cases expression exp
      ...
      (superfieldref-exp (exp1 f-name)
                         (let ([the-object (value-of exp1 env)])
                           (let ([the-ref (find-superfield-ref f-name the-object)])
                             (deref the-ref))))
                         ...
                         (a-class (s-name f-names m-env)
                                  (cases class (lookup-class s-name)
                                    (a-class (s-name f-names m-env) ;; 找到父类的定义
                                             (let ([maybe-ref (lookup-field f-name f-names fields)])
                                               (if maybe-ref maybe-ref
                                                   (eopl:error "~a does not exist in superclass!\n" f-name)))))))))))
(define lookup-field
  (lambda (f-name f-names fields)
    (if (null? f-names) #f
        (if (eqv? f-name (car f-names)) (car fields)
            (lookup-field f-name (cdr f-names) (cdr fields))))))
