...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    ;; 新的instance表达式
    (expression
     ("instanceof" expression identifier) instance-exp)
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      ;; 处理新的instance表达式
      (instance-exp (exp1 id)
                    (let ([val (value-of exp1 env)])
                      (cases object val
                             (an-object (name field) (if (lookup-name-in-family id name)
                                                         (bool-val #t)
                                                         (bool-val #f))))))
	   )))
(define lookup-name-in-family
  (lambda (name class-name)
    (if class-name
        (if (eqv? name class-name) #t
            (cases class (lookup-class class-name)
              (a-class (s-name f-name method-env)
                       (lookup-name-in-family name s-name))))
        #f)))
...
(define-datatype object object?
  (an-object
   (class-name identifier?) ;; 一个object
