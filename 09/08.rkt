  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (fieldref-exp (exp1 id)
                    (let ([a-ref (find-field-ref id (value-of exp1 env))])
                          (deref a-ref)))
      
      (fieldset-exp (exp1 f-name exp2)
                   (let ([a-ref (find-field-ref f-name (value-of exp1 env))])
                     (setref! a-ref (value-of exp2 env))))
                    
	   )))

(define find-field-ref
  (lambda (search-field the-object)
