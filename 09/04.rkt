  ...
  (lambda (pgm)
    ...
    (cases program pgm
      (a-program (class-decls body)
                 (initialize-class-env! class-decls) ;; 这玩意居然是一个全局的变量
                 (value-of body (init-env))))))

(define-datatype object object?
  (an-object
   (instance class?) ;; 这代表的是一个实例,好吧，其实也挺不错的
...
(define-datatype class class? ;; 好吧，这就是所谓的class
  (a-class
...
(define find-method-m
  (lambda (class-instance name)
    (cases object class-instance
      (an-object (class-info fields)
                 (let ([m-env (class->method-env class-info)])
...
(define object->class-name
  (lambda (obj)
    (cases object obj
	   (an-object (class-instance fields)
                      (cases class class-instance
                        (a-class (s-name f-names m-env) s-name))))))
