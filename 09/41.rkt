#lang eopl
;; 这个确实是非常大的一个工程,但是我放在多个文件里的话,会出现非常多的莫名其妙的错误,所以,我将这些东西都堆积到了一个文件里面。
;; 题目的要求是扩展type checker
  ...
  (lambda (exp tenv)
    (cases expression exp
      ...
      (fieldref-exp (exp id)
                    (let ((obj-type (type-of exp tenv)))
                      (if (class-type? obj-type)
                          (if (field-exist? (type->class-name obj-type) id)
                              (int-type)
                              (eopl:error "field ~a doesn't exist!\n" id))
                          (eopl:error "~a is not a class!\n" exp))))
                         
      (fieldset-exp (exp1 f-name exp2)
                    (letrec ([obj-type (type-of exp1 tenv)]
                             [class-name (type->class-name obj-type)])
                      (if (class-type? obj-type)
                          (if (field-exist? class-name f-name)
                              (let ([exp-type (type-of exp2 tenv)])
                                (if (is-subtype? exp-type (get-field-type class-name f-name))
                                    (int-type)
                                    (eopl:error "~a's type do not correspond with field's type!\n" exp2)))
                              (eopl:error "field ~a doesn't exist!\n" f-name))
                          (eopl:error "~a is not a class!\n" exp1))))
...
(define get-field-type
  (lambda (c-name f-name)
    (let ([class (lookup-static-class c-name)])
      (let ([field-names (static-class->field-names class)]
            [field-types (static-class->field-types class)])
        (let find-type ([names field-names]
                        [types field-types])
          (if (null? names) #f
              (if (eqv? (car names) f-name) (car types)
                  (find-type (cdr names) (cdr types)))))))))
    
(define field-exist?
  (lambda (c-name field-name)
    (let ([class (lookup-static-class c-name)])
      (let ([field-names (static-class->field-names class)])
        (eopl:printf "field-names -> ~a\n" field-names)
        (if (member field-name field-names) #t #f)))))
...
(define check-class-decl! ;; 检查类的定义
  (lambda (c-decl)
    (cases class-decl c-decl
           (an-interface-decl (i-name abs-method-decls) ;; 这里是接口的定义,接口的定义一定是通过的嘛。
                              #t)
...
(define add-class-decl-to-static-class-env! ;; 将类型的定义转化成为static class env
  (lambda (c-decl)
    (cases class-decl c-decl
           (an-interface-decl (i-name abs-m-decls) ;; 接口的话
                              (let ((m-tenv
...
class c1 extends object
 field int a
 field bool b
 method int initialize() begin set a = 1; set b = zero?(0); 3 end
