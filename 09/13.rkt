...
(define-datatype method method?
  (a-method
   (type identifier?) ;; 这个东西主要用来判断method的类型
...
(define apply-method
  (lambda (m self args)
    (cases method m
      (a-method (m-type vars body super-name field-names)
                (value-of body
...
(define-datatype class class? ;; class 里面到底包含了一些什么东西呢？
  (a-class
   (super-name (maybe identifier?)) ;; 父类的名称
   (field-names (list-of identifier?)) ;; field的名称
   (method-env method-environment?)))

;;;;;;;;;;;;;;;; method environments ;;;;;;;;;;;;;;;;

;; a method environment looks like ((method-name method) ...)
(define method-environment? ;; 所谓的method-environment就是这个玩意啊！
  (list-of
   (lambda (p)
     (and
      (pair? p)
...
;; method-exist-in-class-and-is-final?
;; 说得简单明了，那就是，这个方法是否在类中存在，并且为final类型
(define method-exist-in-class-and-is-final?
  (lambda (c-name name)
    (let ([m-env (class->method-env (lookup-class c-name))])
      (let ([maybe-pair (assq name m-env)])
        (if (pair? maybe-pair)
            (cases method (cadr maybe-pair)
              (a-method (m-type vars body s-name field-names)
                        (if (eqv? 'final m-type) #t #f)))
            #f)))))
     ...
     (lambda (m-decl)
       (cases method-decl m-decl
         (a-method-decl (method-name vars body)
                        (if (method-exist-in-class-and-is-final? super-name method-name)
                            (eopl:error "~a is a final method!\n" method-name)
                            (list method-name
                                  (a-method 'ordinary vars body super-name field-names)))) ;; 普通类型的method
         (final-method-decl (method-name vars body)
                            (if (method-exist-in-class-and-is-final? super-name method-name)
                                (eopl:error "~a is a final method!\n" method-name)
                                (list method-name
                                      (a-method 'final vars body super-name field-names)))) ;; final 类型的method
       ))
     m-decls)))
