#lang eopl
;; 实现我们所谓的函数重载
;; overloading based on the number of method parameters
      ...
      (let-exp (vars exps body)
	   ...
	   (new-object-exp (class-name rands)
			   (let ([args (values-of-exps rands env)]
			     ...
			     (apply-method
			      (find-method class-name 'initialize (length args))
			      obj
			      args)
			     obj))
...
(define find-method
  (lambda (c-name name len) ;; 这里需要找三个参数，一个是类的名称，一个是方法的名称，然后是参数的个数
    (let ([m-env (class->method-env (lookup-class c-name))])
      (letrec ([find-in-env
                (lambda (name m-env)
                  (if (null? m-env) (report-method-not-found name)
                      (let ([a-meth (car m-env)])
                        (let ([m-name (car a-meth)]
                              [m-meth (cadr a-meth)])
                          (if (eqv? name m-name)
                              (cases method m-meth
                                (a-method (vars body s-name f-names)
                                          (if (eqv? len (length vars)) m-meth
                                              (find-in-env name (cdr m-env)))))
                              (find-in-env name (cdr m-env)))))))])
        (find-in-env name m-env)))))
...
")
;; ==> (list-val (num-val 0) (num-val -1) (num-val 3))
