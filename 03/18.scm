  ...
  (lambda (exp env)
    (cases expression exp
	   ...
	   (let-exp (var exp1 body)
		    (begin
		     ;; (let ((val1 (value-of exp1 env))) (print (extend-env var val1 env)))
		      (let ((val1 (value-of exp1 env)))
			(value-of body
	   ...
	   (let*-exp (var exp1 body)
		     (begin
		      ...
		      ; (print exp1)
		      ; (print env)
		      ; (print (eval&extend-env var exp1 env))
		       (value-of body (eval&extend-env var exp1 env))))
	   ...
	   (unpack-exp (var val body)
		       (let [(plist (value-of val env))]
			   (begin
			     ;(print plist)
			     ;(print (expval-car plist))
			     ;(print (add-into-env var plist env))
			     (value-of body (add-into-env var plist env)))))
	   )))
(define (add-into-env vlist plist new-env)
  (let [(cond1 (null? vlist))
	(cond2 (expval->bool (expval-null? plist)))]
    (cond
     [(and cond1 cond2) new-env]
     [(and (not cond1) (not cond2))
      (add-into-env (cdr vlist) (expval-cdr plist)
		    (extend-env (car vlist) (expval-car plist) new-env))]
     [else (error "Bad unpack expression--the number of var and the number of val is not equal!")])))

(define (eval&extend-env var val new-env)
  (if (null? var) new-env
      (eval&extend-env (cdr var) (cdr val)
		       (extend-env (car var) (value-of (car val) new-env) new-env))))
