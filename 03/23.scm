...
;; test code
;; foctorial procedure
(run "let fac = proc (maker) proc(x) 
                if zero? (x) then 1 else *(((maker maker) -(x, 1)), x) 
                 in 
                  let factorial = proc (x) ((fac fac) x) 
                    in 
                     (factorial 4) ")
...
(run "let makenult =  proc (maker)
                        proc (x)
                         proc (y)
                           if zero? (x) then 0 else -((((maker maker) -(x, 1)) y), -(0, y))
                            in
                              let times = proc (x) proc (y) (((makenult makenult) x) y)
                               in
                                 ((times 3) 8)")
                         ...
                         proc (y)
                           if zero? (x) then 0 else -((((maker maker) -(x, 1)) y), -(0, y))
                            in
                              let times = proc (x) proc (y) (((makenult makenult) x) y)
                               in
                                  let fac = proc (maker) proc (x) 
                                             if zero? (x) then 1 else ((times x) ((maker maker) -(x, 1)))
                                    in
                                        let factorial = proc (x) ((fac fac) x)
                                      in
                                         (factorial 5)")
