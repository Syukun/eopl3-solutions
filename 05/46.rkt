#lang eopl
(require "./base/scheduler.rkt")
(require "./base/store.rkt")
(require "./base/queue.rkt")
(define trace #f)
;; 这道题目主要是关于时间片的东西
;; 其中的一个想法是，调用yield的时候记录下来剩余的时间片，然后下次运行该thread的时候将时间片设置为上次剩余的时间片
    ...
    (cases expression exp
      ...
      (yield-exp ()
                 (place-on-ready-queue!
                  (lambda () (begin
                               (set-remaining-time! the-time-remaining)
                               (eopl:printf "yield-exp :\n the-remaining-time -> ~a\n\n" the-time-remaining)
                 ...
                 ;;           (set-remaining-time! the-time-remaining)
                 ;;           (apply-cont cont (num-val 99)))))
                 ;; 由于闭包的性质，可以保证上面函数的the-time-remaining恰好就是剩余的时间。
                 ;; 还有办法吗？有的，很多方法！
                 (run-next-thread))
  ...
  (lambda (cont val)
    (if (time-expired?) 
        ...
        (begin
          ...
          ;(eopl:printf "apply-cont :\n the-time-remaining -> ~a\n\n" the-time-remaining)
