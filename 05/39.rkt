#lang eopl
;; 好吧！题目的意思是这样的，我们调用raise之后，一般的，异常会向上传播，直到找到了一个try-cont为止，然后从try-cont的handle-exp语句处开始执行
;; 现在题目要求说，我们可不可以实现这样一种语言，从抛出异常的位置开始执行，不过，这玩意有什么意义呢？
;; 好吧，需求确实不太明确，要怎么弄呢？先放在这里吧！
    ...
    (cases continuation cont
      ...
      (div2-cont (val1 saved-cont)
                 (let [(n1 (expval->num val1))
                       (n2 (expval->num val))]
                   (if (= 0 n2) (begin
                                  (eopl:printf "divided by 0 !\n")
                                  (apply-cont (raise1-cont saved-cont) val))
