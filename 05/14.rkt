...
;; 这道题目主要的是求cont的最大深度，毕竟，cont的定义是嵌套的。
...
(define apply-cont
  (lambda (cont val)
    (begin
      (eopl:printf "continuation的深度是 : ~a ~%" (continuation-depth 0 cont))
...
;; new stuff, to compute the depth of a continuation
(define continuation-depth
  (lambda (saved-depth cont)
    (cases continuation cont
      (end-cont ()
                (+ saved-depth 1)) ;; 如果到了这个cont，那么算是到底了。
      (zero1-cont (saved-cont)
                  (continuation-depth (+ saved-depth 1) ;; 递归调用，子cont的深度加上我这个cont的深度，余下的都很类似。
                                     saved-cont))
      (let-exp-cont (v b env saved-cont)
                    (continuation-depth (+ saved-depth 1)
                                       saved-cont))
      ...
      (diff2-cont (v1 saved-cont)
                  (continuation-depth (+ saved-depth 1)
                                     saved-cont))
      (mult1-cont (e2 env saved-cont)
                  (continuation-depth (+ saved-depth 1)
                                     saved-cont))
      (mult2-cont (v1 saved-cont)
                  (continuation-depth (+ saved-depth 1)
                                     saved-cont))
      (rator1-cont (rands env saved-cont) 
                   (continuation-depth (+ saved-depth 1)
                                      saved-cont))
      (rand1-cont (rands env saved-cont rator acc) 
                  (continuation-depth (+ saved-depth 1)
                                     saved-cont)))))
