...
;; Add multideclaration let to this interpreter.
    ...
    (cases continuation cont
      ...
      (diff2-cont (val1 cont)
                  (let ((num1 (expval->num val1))
                    ...
                    (apply-cont cont
                                (num-val (- num1 num2)))))
      (let1-cont (var-list exp-list body env cont acc)
                 (begin
                   ;(print "acc => " acc)
                   (if (null? exp-list)
                       (value-of/k body (extend-env-list var-list (cons val acc) env) cont)
                       (value-of/k (last-element exp-list) env
                                   (let1-cont var-list (except-last-element exp-list) body env
                                              cont (cons val acc))))))  
      )))
(define (extend-env-list var-list val-list env)
  (begin
    ;(print "val-list -> " val-list)
    (if (null? var-list) env
        (extend-env-list (cdr var-list) (cdr val-list)
  ...
  (lambda (exp env cont)
    (cases expression exp
      ...
      (let-exp (var-list exp-list body)
                 (value-of/k (last-element exp-list) env
                             (let1-cont var-list (except-last-element exp-list)
                                        body env cont '())))
